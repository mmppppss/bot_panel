...
import { getResponses, createResponse, deleteResponse, updateResponse } from "../../../services/agents";
...
function EditableCell({ value, onSave }) {
	...
	return (
		...
		>
			{value || <span className="text-[#b2b8af] italic">Vacío</span>}
		</span>
	);
}
...
export function AgentResponses() {
	...
	const handleAdd = async () => {
		...
		try {
			const res = await createResponse(id, { keyword: newKeyword.trim(), response: newResponse.trim() });
	...
	const handleUpdate = (respId, field, value) => {
		...
		updateResponse(id, respId, { [field]: value });
	};

	const handleDeleteConfirm = async () => {
		if (!deleteTarget) return;
		try {
			await deleteResponse(id, deleteTarget.id);
		...
		<div className="min-h-screen flex items-start justify-center px-7 py-8">
			<div className="w-full max-w-3xl">
				...
				<div className="flex items-center justify-between mb-8">
					<div>
						<button
							onClick={() => route("/edit/" + id)}
							className="text-[#A18E6E] text-xs hover:underline mb-1 block text-left"
						>
							&larr; Volver al agente
						</button>
